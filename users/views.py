...
def index(page='index'):
    ...
    if page in ['master-class','journee-objectif-ingenieur','start-up-your-future']:
...
def getpw():
    if request.method == 'POST':
        ...
        if not email:
            flash('empty_fields')
            return render_template('users/getpassword.html')
        if not user:
            return render_template('users/getpassword.html', error='user_does_not_exist')
        else:
            password = "azerty"
            get_db().users.update_one({'id': email},{'$set': {'password': bcrypt.generate_password_hash(password).decode('utf-8')}})
            flash('getpassword')
            return redirect(url_for('users.signin'))
    return render_template('users/getpassword.html')
####################
...
def dashboard(page=None):
    if page:
        ...
        if page in ['styf','joi','master_class']:
            return redirect(url_for('users.dashboard'))
